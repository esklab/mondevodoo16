...
class cinetpayController(http.Controller):
    ...
    def cinetpay_callback_from_redirect(self, **call_kw):

        cinetpay_api = request.env['payment.provider'].sudo().search([('code','=','cinetpay')], limit=1)
        _data = json.loads(request.httprequest.data)

        token = _data['token']

        data = jwt.decode(token, cinetpay_api.cinetpay_api_key, algorithms=["HS256"])
    ...
    def cinetpay_return_from_redirect(self, **call_kw):
